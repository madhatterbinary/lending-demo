...
class RedirectForm extends PureComponent {
  ...
  render() {
    ...
    const { redirect_payload: redirectPayload, post_url: postUrl } = pageData;

    return (
      <div className='form-container'>
        <Spinner />
        <form id='redirectWorldpayForm' action={ postUrl } method='POST' style={{ display: 'none' }}>
          <input type='hidden' value={ (redirectPayload || {}).MD } name='MD' />
          <input type='hidden' value={ (redirectPayload || {}).PaReq } name='PaReq' />
          <input type='hidden' value={ (redirectPayload || {}).TermUrl } name='TermUrl' />
